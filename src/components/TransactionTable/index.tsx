                ...
                <tbody className="bg-white divide-y-8 divide-gray-100">
                    {transactions.map((transaction) => (
                        <tr key={transaction.id}>
                            ...
                            <td className={`px-8 py-4 whitespace-nowrap font-light ${transaction.price < 0 ? 'text-outcome-value' : 'text-income-value'}`}>
                                R$ {transaction.price.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                            <td className="px-8 py-4 whitespace-nowrap text-gray-400 font-light">{transaction.categorie}</td>
